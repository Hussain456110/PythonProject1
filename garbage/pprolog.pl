...
user(Ali, 30, intermediate, weight_loss, [none]).
user(Hammad, 50, beginner, flexibility, [joint_pain]).
user(Manan, 25, advanced, muscle_gain, [none]).
user(Rafay, 35, beginner, endurance, [asthma]).
...
% Query examples:
% ?- hydration_reminder(Ali, Reminder).
% ?- active_recovery_day(Hammad, RecoveryActivity).
% ?- workout_music(Manan, Playlist).
% ?- workout_schedule(Rafay, Schedule).
% ?- calculate_bmi(Ali, 1.75, 70, BMI, Category).
% ?- fitness_gadgets(Ali, Gadgets).
% ?- goal_progress(Hammad, 10, 5, Progress).
% ?- exercise_for_mood(energetic, Exercise).
% ?- partner_compatibility(Ali, Manan, Compatibility).
% ?- reward_system(Manan, 15, Reward).
